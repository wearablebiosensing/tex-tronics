package edu.uri.wbl.bledemo.ble;
...
import edu.uri.wbl.bledemo.ble.GattAttributes.GattDescriptors;

/**
 * Created by Matthew Constant on 11/28/2017.
 *
 *
 */
...
public class BluetoothLeService extends Service {
    ...
    public void enableNotify(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        ...
        // AndyP
        // Added the write to the descriptor
        BluetoothGattDescriptor descriptor = characteristic.getDescriptor(GattDescriptors.NOTIFICATION_DESCRIPTOR);
        if (descriptor == null) {
            Log.w(TAG,"Characteristic does not have Notification Descriptor");
            return;
        } else {
            descriptor.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE);
            if (!gatt.writeDescriptor(descriptor)) {
                Log.w(TAG,"Could not write to descriptor");
                return;
            } else {
                Log.w(TAG,"Writing to descriptor...");
            }
        }
    }
