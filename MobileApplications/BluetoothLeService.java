...
public class BluetoothLeService extends Service {
    ...
    {
        // Andrew: Copied from BleConnectionService
    ...
    {
        // Andrew: Started writing Request Read
        // Copied from BLEConnection Service
        Intent intent = new Intent(context, BleConnectionService.class);
        // Action changed from read_characteristic to request_read
    ...
    private HashMap<String, BluetoothGatt> mGattList;       // Andrew: Created hashmap to store connected gatts
    ...
    public void onCreate() {
        ...
        // Andrew: Initialize hashmap
        mGattList = new HashMap<>(7);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        switch (action) {
            ...
            case CONNECT:
                ...
                break;
            case DISCONNECT:
                // Andrew: Copied from BLEConnectionService Disconnect
                Log.d(this.getClass().getSimpleName(), "ACTION: Disconnect");
                if(!intent.hasExtra(EXTRA_DEVICE)) {
                    Log.d(this.getClass().getSimpleName(), "Invalid Action Intent");
                    return START_NOT_STICKY;
                }
                // ------Copied from above Connect Case------
                bluetoothLeModel = (BluetoothLeModel) intent.getSerializableExtra(EXTRA_DEVICE);
                if(bluetoothLeModel == null) {
                    Log.w(this.getClass().getSimpleName(), "NULL BT Model");
                    break;
                }
            ...
            case REQUEST_READ:
                ...
                // Andrew: Starting writing request read
                if(!intent.hasExtra(EXTRA_DEVICE)) {
                    Log.d(this.getClass().getSimpleName(), "Invalid Action Intent");
                    return START_NOT_STICKY;
                }
                ...
                //------------------------------------------
                // Wow, what a crazy line of code. Woah!
                UUID serviceUUID = UUID.fromString(intent.getStringArrayExtra(EXTRA_SERVICE).toString());
                if(serviceUUID == null)
                {
                    Log.w(this.getClass().getSimpleName(), "NULL BT SERVICE");
                    break;
                }
                UUID charUUID = UUID.fromString(intent.getStringArrayExtra(EXTRA_CHARACTERISTIC).toString());
                if(charUUID == null)
                {
                    Log.w(this.getClass().getSimpleName(), "NULL BT CHARACTERISTIC");
                    break;
                }
                // Everything is all good, so lets request that read, huh?
                requestRead(bluetoothDeviceAddress, serviceUUID, charUUID);
                break;
            case REQUEST_WRITE:
    ...
    private void connect(String bluetoothDeviceAddress) {
        ...
        // Andrew: After connected, put it into list
        mGattList.put(bluetoothDeviceAddress, gatt);
    ...
    private void disconnect(String bluetoothDeviceAddress) {
        // Andrew: Wrote disconnect method
        // If there is no device to disconnect from
        if(bluetoothDeviceAddress == null)
        {
            Log.w(this.getClass().getSimpleName(), "Could not Disconnect: NULL BT ADDR");
            return;
        }
        // Inspired from BleConnectionService / Current Smart Glove
        BluetoothGatt gatt = mGattList.get(bluetoothDeviceAddress);
        // Checks to see if the gatt is connected
        if(gatt == null)
        {
            Log.w(this.getClass().getSimpleName(), "Could not Disconnect: NULL Gatt");
            return;
        }

        // Disconnect from device, then remove it from list
        gatt.disconnect();
        mGattList.remove(bluetoothDeviceAddress);
    ...
    {
        // Andrew: Started writing request_read
        // Checks to see if each parameter contains the appropriate information
        if(bluetoothDeviceAddress == null)
        {
            Log.w(this.getClass().getSimpleName(), "Could not read: NULL BT ADDR");
            return;
        }
        if(serviceUUID == null)
        {
            Log.w(this.getClass().getSimpleName(), "Could not read: NULL BT SERVICE");
            return;
        }
        if(characteristicUUID == null)
        {
            Log.w(this.getClass().getSimpleName(), "Could not read: NULL BT CHAR");
            return;
        }
        // I'm having this get the characteristic from the service, then reading from it. Is this right?
        BluetoothGatt gatt = mGattList.get(bluetoothDeviceAddress);
    ...
    // Andrew: Using readCharacteristic method inspired by Smart Glove master
    public boolean readCharacteristic(BluetoothGatt bluetoothGatt, BluetoothGattCharacteristic characteristic) {
        if (mBluetoothAdapter == null) {
            Log.w(this.getClass().getSimpleName(),"Bluetooth Adapter not initialized");
            return false;
        }

        if (bluetoothGatt == null) {
            Log.w(this.getClass().getSimpleName(),"Bluetooth Device's Gatt Server not Connected");
            return false;
        }

        if (characteristic == null) {
            Log.w(this.getClass().getSimpleName(),"Invalid Characteristic");
            return false;
        }

        Log.w(this.getClass().getSimpleName(),"Reading Characteristic (" + characteristic.getUuid() + ")...");
